...
public class BoardController {
	...
	public String getBoardList(@RequestParam(value = "page", defaultValue = "1") int page,
			...
			@RequestParam(value = "mode", required = false, defaultValue = "all") String mode, Model model) {
		log.debug("getBoardList() called");
		log.debug("페이지당 글수:{}, 페이지이동 링크수{}, page : {}, type : {}, searchWord : {}, mode : {}", countPerPage,
				pagePerGroup, page, type, searchWord, mode);
	...
	public String writeForm(BoardVO board, @AuthenticationPrincipal UserDetails user // 로그인한 사용자 정보 전달
	) {
	 ...
	 */
	@GetMapping("delete")
	public String delete(int board_id, @AuthenticationPrincipal UserDetails user) {
		// 해당번호의 글 조회
	...
	public String replyWrite(ReplyVO reply, @AuthenticationPrincipal UserDetails user) {
